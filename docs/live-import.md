...
## Current read-only preflight refresh — 2026-09-20

The refreshed authorized-development preflight found all four unmanaged 0.1.0.0 solutions, 24 bound Custom APIs, 14 active-key companion tables, 41 correct lifecycle/acquisition registrations, and all expected connection references. All seven framework/reference flows are Draft, with zero event callback registrations. The TestCoordinator Draft definition was separately aligned to include its checked-in cleanup branch; no flow was activated. `ready=false` remains correct because clean managed import, licensing/capacity, target-user privileges, connection ownership, event activation, and customer-flow activation are still unverified. See [redacted refresh evidence](evidence/live-preflight-2026-09-20-refresh.json).
...
A live MCP run initially failed when cancellation attempted Queued-to-Exception. The revised runtime uses OnHold for unstarted cancelled items. The original proof and a fresh 2026-09-19 run both passed: Cancelled run and result, native OnHold, zero attempts, and identical receipt replay after the client disconnected. The subsequent active-worker and concurrent probes also passed their synthetic tenant criteria, including a structured `VERSION_CONFLICT` for the losing native transaction. See [original evidence](evidence/test-cancellation-2026-09-12.json), [fresh evidence](evidence/test-cancellation-2026-09-19.json), [active-worker evidence](evidence/test-cancellation-active-2026-09-19.json), [concurrent evidence](evidence/test-cancellation-concurrent-2026-09-19.json), and [reproduction](test-cancellation.md). Broader coordinator recovery, separate-user authorization, managed installation and production-like flows remain open.
...
## Full contract snapshot and completion replay

Attempts now persist the effective contract schema, ID, dialect and hash, alongside the existing policy snapshot. Older attempts explicitly deserialize with no full contract snapshot. Both acquisition paths and legacy deserialization have regression coverage. The updated package passed all 211 local tests and eight archive round trips before being pushed to the development environment.

The [installed proof](evidence/contract-completion-2026-09-12.json) rejected in-place contract changes and retained the attempt snapshot after later policy edits. Completion with a deliberately discarded successful response replayed from one persisted command receipt, preserved one business record and native completion, and emitted one pending outbox event using the attempt's original policy. Changed arguments with the same request were rejected. No notification was sent; the original queue policy was restored. [Reproduction and limitations](contract-completion-proof.md) distinguish controlled response discard from an actual connector outage.
