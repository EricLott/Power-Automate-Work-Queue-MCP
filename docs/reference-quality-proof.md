...
For a fresh isolated installed run, `scripts/prove_reference_quality_autonomous.py` temporarily binds and activates only `ProcessOne`, `SweepQueue` and `TestCoordinator` against an idle synthetic queue, suspends the coordinator's automatic cleanup branch while it reads independent business evidence, starts the unchanged six-case manifest, invokes and verifies test-owned cleanup only after a complete pass, and restores every changed flow to its original clientdata and Draft state. It never activates `Intake`, `OnQueueChanged` or `EmailSender` and never contacts a mailbox or external destination. The raw output belongs under ignored `artifacts/live`; publish only redacted evidence.
