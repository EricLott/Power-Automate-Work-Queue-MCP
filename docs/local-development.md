...
It checks the pinned local artifacts and reads installed versions, API plug-in bindings, synchronous guard/image configuration, table keys/concurrency, connection mappings, flow states, and `workqueueitem` callback registrations. High-cardinality metadata is read with bounded bulk OData filters so the preflight does not launch one CLI process per API or registration step. An Active `OnQueueChanged` flow without at least one matching callback registration makes `observableComplete` false; the flow row alone is not proof that an event subscription exists. `observableComplete` describes those observed components; `ready` remains false while licensing, target privileges, and connection ownership need verification. It neither imports nor activates anything, and it excludes flow clientdata. The bound MCP `preflight_installation` tool exposes this same read-only check and validates that the result reports no writes or clientdata access. The latest bound-MCP result is [recorded here](evidence/mcp-preflight-2026-09-19.json).
