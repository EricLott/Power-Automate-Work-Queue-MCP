...
The 2026-09-19 probe produced one durable cancellation winner and a safe terminal reconciliation, but the losing worker call surfaced only as `DATAVERSE_CLI_FAILED`, so `nativeConcurrencyVerified` remains false in [the redacted evidence](evidence/test-cancellation-concurrent-2026-09-19.json). Exposing the structured native conflict through the authenticated transport is still required before closing the concurrent-cancellation criterion.
