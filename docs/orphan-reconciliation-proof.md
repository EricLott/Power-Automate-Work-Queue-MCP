...
The runtime watchdog quarantines a native `Processing` item that has no framework item context. The bounded maintenance scan changes the native item to `Exception` and does not create an attempt or business output. The behavior is covered by `BoundaryTests.WatchdogQuarantinesNativeProcessingItemWithoutFrameworkContext`.
