...
export async function planInstallation({bindingPath='',packageDir=path.join(root,'artifacts/packages'),manifest:givenManifest,release:givenRelease}={}){
  ...
  const packages=await Promise.all(entries.map(async entry=>{try{const actual=digest(await readFile(path.join(packageDir,entry.file)));return {file:entry.file,expectedSha256:entry.sha256,actualSha256:actual,status:actual.toLowerCase()===entry.sha256.toLowerCase()?'verified-local':'tampered'};}catch(error){if(error.code!=='ENOENT')throw error;return {file:entry.file,expectedSha256:entry.sha256,actualSha256:null,status:'missing'};}}));
