...
test('deployment result identifies target, durable record and affected IDs',async()=>{const root=await mkdtemp(path.join(os.tmpdir(),'dep-'));roots.push(root);const bp=path.join(root,'b.json'),sp=path.join(root,'s.json');await writeFile(bp,JSON.stringify(b));await writeFile(sp,'{}');const env={QMCP_ENVIRONMENT_BINDING:bp,QMCP_DEPLOYMENT_SETTINGS:sp,QMCP_APPROVED_DEPLOYMENT_PLAN:'1'.repeat(64)},args={planHash:env.QMCP_APPROVED_DEPLOYMENT_PLAN,requestId:'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb'};const first=await applyDeployment(args,{root,env,launch:()=>{}});assert.deepEqual(first.environment,{organizationId:b.organizationId,environmentUrl:b.environmentUrl});assert.deepEqual(first.affectedIds,[]);assert.equal(first.deploymentRecord,path.join(root,'artifacts/deployments',args.requestId+'.launch.json'));await writeFile(path.join(root,'artifacts/deployments',args.requestId+'.json'),JSON.stringify({requestId:args.requestId,organizationId:b.organizationId,environmentUrl:b.environmentUrl,planHash:args.planHash,status:'ImportedAwaitingAcceptance',affectedSolutions:[{solutionid:'core-id'},{solutionid:'testing-id'}]}));const status=await deploymentStatus(args,{root,env});assert.deepEqual(status.affectedIds,['core-id','testing-id']);assert.equal(status.deploymentRecord,path.join(root,'artifacts/deployments',args.requestId+'.json'));});
