...
try {
  ...
  const requiredResources = ['qmcp://wq/architecture/v0.1', 'qmcp://wq/flow/ProcessOne/v1'];
  for (const uri of requiredResources) {
    const resource = await client.readResource({ uri });
    if (!resource.contents?.length) throw new Error('MCP_RESOURCE_READ_FAILED');
  }
  ...
  console.log(JSON.stringify({ classification: 'read-only-mcp-stdio-probe', transport: process.env.QMCP_DATAVERSE_TRANSPORT || 'cli', toolCount: tools.tools.length, resourceCount: resources.resources.length, resourcesRead: requiredResources, target: result.target, organizationId: result.identity.organizationId, mutations: 0 }, null, 2));
